...
RSpec.describe Slack do
  context 'exception logs' do
    it 'formats using the exception template' do
      ...
      expect(attachments).to eq [{
        "fields"=> [
          {
            "short"=>true,
            "title"=>"Server",
            "value"=>"cohortpay-demo:app/web.1"
          },
          {
            "short"=>true,
            "title"=>"Response",
            "value"=>"422 after 34ms"
          }
        ],
        "mrkdwn_in"=>["pretext"],
        "pretext"=>[
          "*POST* /admin/users/identity_push.json",
          "*Admin::UsersController* > identity_push"
        ].join("\n")
      }, {
        "color"=>"danger",
        ...
        "mrkdwn_in"=>["text"],
        "text"=>[
          "*Validation failed* Dob must be at least four years old",
          "```",
          "* app/admin/system/users.rb: 258 in block (3 levels) in <top (required)>",
          "* app/admin/system/users.rb: 256 in block (2 levels) in <top (required)>",
          "  lib/rack_headers.rb     :  72 in _call",
          "  lib/rack_headers.rb     :  68 in call",
          "```"
        ].join("\n")
      }]
    end
    ...
    it 'reports on database errors' do
      ...
      expect(attachments).to eq [{
        'fields' => [{
          'short' => true,
          'title' => 'Server',
          'value' => 'cohortflow:app/web.1'
        }, {
          'short' => true,
          'title' => 'Response',
          'value' => '500 after 244ms'
        }],
        'mrkdwn_in' => ['pretext'],
        'pretext' => [
          '*PUT* /admin/organisations/ap:11122/products.json',
          '*Admin::Organisations::ProductsController* > update'
        ].join("\n")
      }, {
        'color' => 'danger',
        ...
        'mrkdwn_in' => ['text'],
        'text' => [
          "*PG::NotNullViolation: ERROR* null value in column \"app_features\" violates not-null constraint",
          "```",
          "* app/controllers/admin/organisations/products_controller.rb:  10 in update",
          "  lib/rack_headers.rb                              :  71 in _call",
          "  lib/rack_headers.rb                              :  67 in call",
          "```"
        ].join("\n")
      }]
    end
  ...
  context 'heroku debug logs' do
    it 'formats using the debug template' do
      log_lines = %{
  ...
  context 'combination of log types' do
    ...
    it 'things' do
      ...
      Slack.message({'text' => 'Error Code: 500 - 20181120150057'})
    end
  end
end
